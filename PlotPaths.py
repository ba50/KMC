...
class PlotPaths(app.Canvas):
    def __init__(self, positions):
        ...
        a_position = positions[:, 0:3]
        for i in range(1, 3):
            a_position = np.insert(a_position, -1, positions[:, (3*i):(3*i+3)], 0)

        for i in range(1, a_position.shape[0]*2-2):
            if i % 2 == 0:
                a_position = np.insert(a_position, i, a_position[i-1], 0)

        for i in range(np.floor(a_position.shape[0]/2).astype(np.int)):
            a_id.append(i)
            if i % (positions.shape[0]+1) == 0:
                a_id.append(i)
            else:
                a_id.append(i+1)
...
if __name__ == '__main__':
    ...
    oxygen_path = np.memmap('update_vector.bin', dtype='float32', mode='r+', shape=(shape[0], shape[1]*3-2))
    oxygen_path = oxygen_path[:10]
