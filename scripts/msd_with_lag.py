...
def worker(inputs):
    ...
    msd_list = [
        chunk[["x", "y", "z"]].diff(periods=lag).reset_index(drop=True)
        for _, chunk in simulation_frames.groupby("atom_ids")
    ]

    mean_msd = pd.DataFrame(
        {"time": field_data["time"][1:], "x": 0.0, "y": 0.0, "z": 0.0}
    )
    for atom_ids in range(n_atoms):
        msd_list[atom_ids]["time"] = field_data["time"]
        msd_list[atom_ids] = msd_list[atom_ids].dropna()
        mean_msd[["x", "y", "z"]] = mean_msd[["x", "y", "z"]].add(
            msd_list[atom_ids][["x", "y", "z"]]
        )
    ...
    plt.ylabel("MSD [au]")
    plt.legend()
    plt.savefig(sim_path / "msd" / f"msd_with_lag_{lag}_freq_{conf.frequency:.2e}.png")
    plt.close()


def msd_with_lag(args):
    sim_path_list = [
        (args.lag, sim) for sim in args.data_path.glob("*") if sim.is_dir()
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lag", type=int, default=1, help="MSD lag")
