...
class GenerateModel:
    ...
    def generate_sphere(self, radius):
        ...
        for index, kation in np.ndenumerate(self.kations):
            position = (
                index[0] * self.cell_size,
                index[1] * self.cell_size,
                index[2] * self.cell_size,
            )
            if to_change:
                test = np.sqrt(
                    (position[0] - center[0]) ** 2
        ...
        for index, anion in np.ndenumerate(self.anions):
            position = (
                index[0] * self.cell_size + self.cell_size * 0.5,
    ...
    def read_file(simulation_path):
        ...
        with simulation_path.open("r") as file_in:
            for line in file_in.readlines()[2:]:
                line = line.split("\t")
                if line[0] == "Bi":
                    bi_base_positions.append(
                        [float(line[1]), float(line[2]), float(line[3])]
                    )
                if line[0] == "Y":
                    y_base_positions.append(
                        [float(line[1]), float(line[2]), float(line[3])]
                    )
                if line[0] == "O":
                    o_base_positions.append(
                        [float(line[1]), float(line[2]), float(line[3])]
                    )

        return (
            np.array(bi_base_positions),
            np.array(y_base_positions),
            np.array(o_base_positions),
        )
