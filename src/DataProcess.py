...
def data_process(inputs):
    ...
    if args.plots:
        print('Save in:', simulation_path / 'plots')
        (simulation_path / 'plots').mkdir(parents=True, exist_ok=False)
   
    if args.o_paths:
        print('Save in:', simulation_path / 'paths')
        (simulation_path / 'paths').mkdir(parents=True, exist_ok=False)

    if args.o_paths or args.plots:
    ...
    if args.plots:
        ...
        plt.savefig(simulation_path / 'plots' / "Jumps.png", dpi=args.dpi)
        ...
        plt.savefig(simulation_path / 'plots' / "Atom.png", dpi=args.dpi)

        loc_index = list(range(0, when_which_where.shape[0], int(when_which_where.shape[0] // args.time_points)))

        plotting.plot_line(save_file=simulation_path / 'plots' / 'Time.png',
        ...
        plotting.plot_line(save_file=simulation_path / 'plots' / 'delta_Time.png',
                           x_list=[range(len(loc_index))],
        ...
        plotting.plot_line(save_file=simulation_path / 'plots' / 'Field.png',
        ...
        time_heat_map = TimeHeatMap(load_data_path=simulation_path)
        time_heat_map.process_data()
